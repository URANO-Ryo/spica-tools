...
The ``topfile`` can be generated with ``json2top``, ``ENM``, (or ``Go`` ) commands (for proteins/nucleic acids).
...
``-Go``
    Enable special handling for proteins with Go model  When this option is used, 
    each protein molecule is treated as an independent topology entry, even when multiple 
    copies (nmol > 1) are specified. This is required for correct indexing of Go
    interactions within each protein molecule. **But, currently, GENESIS doens not check the implemention of this Go SPICA model, and should use ENM model.**
